...
fn main() {
    let words = SENTENCE.split_ascii_whitespace().collect::<Vec<_>>(); // Convert sentence into a vector of words
    let frequency_count = word_frequency_counter(words); // Pass the vector directly
